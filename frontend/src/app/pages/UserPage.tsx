      ...
      <main className="max-w-8xl mx-auto px-3 sm:px-5 py-5 space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm px-3 sm:px-4 py-2.5 sm:py-3">
          <div className="flex flex-wrap items-center gap-x-3 sm:gap-x-6 gap-y-1.5 sm:gap-y-2">
            ...
            <div className="ml-auto flex items-center gap-1.5 bg-slate-50 rounded-lg px-2.5 sm:px-3 py-1.5 border border-slate-200">
              <Store className="w-3.5 h-3.5 text-slate-500" />
              <span className="text-[10px] sm:text-xs text-slate-500">Total: <span className="text-slate-800 font-bold">{stalls.length}</span></span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="px-4 py-3 border-b border-slate-100">
            ...
            <div className="mt-3 relative">
              <div className="relative inline-block" id="map-view-dropdown-wrapper">
                <button
                  id="map-view-selector"
                  onClick={() => setDropdownOpen(o => !o)}
                  style={{ color: '#334155' }}
                  className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold bg-slate-100 hover:bg-slate-200 transition-all duration-200 border border-slate-200 relative z-10"
                >
                  {mapView === 'design' ? (
...
function StatPill({ color, label, count }: { color: string; label: string; count: number }) {
  return (
    <div className="flex items-center gap-1.5 sm:gap-2">
      ...
      <span className="text-[10px] sm:text-xs font-black text-slate-800 bg-slate-100 rounded-md px-1 sm:px-1.5 py-0.5 min-w-[1.25rem] sm:min-w-[1.5rem] text-center">{count}</span>
    </div>
  );
}
